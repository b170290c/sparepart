...
//	$conn = $pdo->open();
	...
	if(isset($_SESSION['user'])){
		try{
			...
			$sql = "SELECT *, products.name AS prodname, category.name AS catname FROM sps.cart LEFT JOIN sps.products ON products.id=cart.product_id LEFT JOIN sps.category ON category.id=products.category_id WHERE user_id= '".$user['id']."' ";
			$params = array();
		...
		if(empty($_SESSION['cart'])){
			...
			while($row = sqlsrv_fetch_array( $_SESSION['cart'], SQLSRV_FETCH_ASSOC)){
				$output['count']++;

				$sql = "SELECT *, products.name AS prodname, category.name AS catname FROM sps.products LEFT JOIN sps.category ON category.id=products.category_id WHERE products.id= '".$row['productid']."' ";
				$params = array();
		   		$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
