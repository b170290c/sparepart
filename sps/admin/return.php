                ...
                <tbody>
                  ...
                  //  $conn = $pdo->open();
                    ...
                    try{
                     ...
                     // $stmt = $conn->prepare("SELECT sales.id AS salesid, details.product_id AS product_id, details.detailid AS detailid, users1.line AS line, users1.plant AS plant, users1.empid AS empid, users1.firstname AS firstname, users1.lastname AS lastname  FROM sales LEFT JOIN details on details.sales_id=sales.id LEFT JOIN rdetails ON details.detailid=rdetails.detailid LEFT JOIN users1 ON users1.id=sales.user_id WHERE details.detail_status= 3 ORDER BY time DESC");
                     //  $stmt->execute();
