...
try{
 ...
 $query="SELECT products.noSerial AS noSerial, products.noSAP AS noSAP, products.noParts AS noParts, products.recdate AS recdate, products.minqnty AS ROP, products.mls AS MLS, products.roq AS ROQ, products.leadtime AS leadtime, products.qnty AS quantity, products.price as price, products.id AS prodid, products.name AS prodname, category.name AS catname, vendor.name AS vename FROM sps.products  LEFT JOIN sps.category ON category.id=products.category_id LEFT JOIN sps.vendor ON vendor.id= products.vendor_id WHERE products.recdate BETWEEN '$start' AND  '$end' ORDER BY products.recdate DESC";
 $params = array();
