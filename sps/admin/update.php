...
$valid = $ownQnty - $qtyok ;
	...
	//accept calc.				
					...
					// $stmt->execute(['newBlnc'=>$newBlnc , 'pid'=>$pid ]);
					 ...
					 // $stmt = $conn->prepare("INSERT INTO ongoing (detailid, salesid, product_id, quantity, dates ) VALUES (:detailid, :sales_id, :product_id, :quantity, :dates)");
					 // $stmt->execute(['detailid'=>$detailid, 'sales_id'=>$sid, 'product_id'=>$pid,  'quantity'=>$bal, 'dates'=>$date]);
						$sql = " INSERT INTO sps.ongoing ([detailid], [salesid], [product_id], [quantity], [dates] ) VALUES ('".$detailid."', '".$sales_id."', '".$product_id."', '".$bal."' , '".$date."') ";
						$stmt = sqlsrv_query($conn , $sql);
