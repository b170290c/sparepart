...
$this->Cell(71 ,5,strtoupper('Low Stock Quantity Report'),0,0,'R');
...
$pdf->SetWidths(Array(10,18,20,15,15,29,20,15,15,15,20,15,15,20,23));
...
try{
 ...
 $query="SELECT products.id AS prodid,  category.name AS catname, vendor.name AS vename,products.name AS prodname, products.description AS description, products.photo AS photo, products.date_view AS date_view, products.price AS price, products.qnty AS quantity, products.minqnty AS ROP, products.mls AS MLS, products.roq AS ROQ, products.leadtime AS leadtime, products.recdate AS recdate, products.idLoc AS idLoc, products.noSerial AS noSerial, products.noSAP AS noSAP, products.noParts AS noParts FROM sps.products LEFT JOIN sps.category ON category.id=products.category_id LEFT JOIN sps.vendor ON vendor.id= products.vendor_id WHERE qnty <= minqnty";
$params = array();
