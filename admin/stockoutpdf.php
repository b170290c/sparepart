...
$this->Cell(71 ,5,strtoupper('Stock Out By Line'),0,0,'R');
...
$pdf->SetWidths(Array(10,25,25,20,20,35,30));
...
try{
 ...
 $query="SELECT  users1.line as uline, sales.sales_date, Sum(details.qtyok) as quantity, Sum(products.price*details.qtyok) AS total, users1.firstname as firstname , users1.lastname as lastname FROM sps.details  JOIN sps.products ON details.product_id=products.id  JOIN sps.sales ON sales.id=details.sales_id  JOIN sps.users1 ON users1.id=sales.user_id WHERE sales.sales_date BETWEEN '$start' AND  '$end' AND details.detail_status=1  GROUP BY line,sales_date,quantity,price,qtyok,firstname,lastname";
 $params = array();
